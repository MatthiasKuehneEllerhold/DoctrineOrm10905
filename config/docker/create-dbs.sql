...
CREATE TABLE `product_default_machine` (`product` VARCHAR(50) NOT NULL, `location` VARCHAR(50) NOT NULL, `product2` VARCHAR(50) NOT NULL, `machine` VARCHAR(50) NOT NULL, PRIMARY KEY (`product`, `location`), INDEX `IDX_D50B8B5CD34A04AD` (`product`), INDEX `IDX_D50B8B5CB2612B5C1505DF84` (`product2`, `machine`));
ALTER TABLE `product_default_machine` ADD CONSTRAINT `default_product` FOREIGN KEY (`product`) REFERENCES `product` (`name`);
ALTER TABLE `product_default_machine` ADD CONSTRAINT `default_product_machine` FOREIGN KEY (`product2`, `machine`) REFERENCES `product_available_machine` (`product`, `machine`);
...
INSERT INTO `product_default_machine` VALUES ('teacup', 'berlin', 'teacup', 'injection molder'), ('teacup', 'essen', 'teacup', 'drillpress');
